...
import { CardDeck, CardColumns, Container, Row, Col, Button} from 'reactstrap'
...
export default class CardStack extends React.Component {
    ...
    constructor(props) {
        super(props)
        this.state = { cardList: [], filterTags: [] }
    }
    ...
    toggleFilter(tag) {
        ...
        if (filterTags.some(e => e === tag)) {
            this.setState({filterTags: this.state.filterTags.filter(e => e !== tag)})
        } else {
            filterTags.push(tag)
    ...
    render() {
        ...
        return <Container >
            <Row><Col style={{alignContent: "center", textAlign:"center"}}>
                <div style={{padding: "0.5rem"}}>
                { tagList.map((tag) => { 
                    return <Button color={ this.checkFilter(tag) ? "primary" : "info" } onClick={() => this.toggleFilter(tag)}>{tag}</Button>
                })
                }
                </div>
            </Col></Row>
            ...
            { cardList.map((item) => {
                    return <Col key={item._id} style={{maxWidth: '100vw', paddingTop: '1rem', paddingBottom: '1rem'}}>
                        <Flippy 
